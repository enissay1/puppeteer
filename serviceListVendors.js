...
// const path = require('/home/app/path');

async function scrapeCdiscountVendors(url, csvTitle, csvFilePath) {
    ...
    try {
        ...
        const csvData = allContentArray.map(titleContent => ({ [csvTitle]: titleContent }));
        console.log(csvData);
        ...
        if (!fileExists) {
            fs.writeFileSync(csvFilePath, `${csvTitle}\n`);
        }

        // Si le fichier existe, append les données
        fs.appendFileSync(csvFilePath, csvData.map(item => item[csvTitle]).join('\n') + '\n');
...
module.exports = scrapeCdiscountVendors;

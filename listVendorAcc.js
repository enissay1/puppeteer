...
(async () => {
    ...
    try {
        ...
        await page.goto('https://www.cdiscount.com/mp-1535-910001793.html');
        ...
        const csvData = allContentArray.map(titleContent => ({ 'ACCESSORIES': titleContent }));
        console.log(csvData);
        //
        // Vérifiez si le fichier existe
        const filePath = path.resolve('/home/app/listOfVendorsAcc.csv');
        ...
        if (!fileExists) {
            fs.writeFileSync(filePath, 'ACCESSORIES\n');
        }
        console.log(filePath);
        // Si le fichier existe, append les données
        fs.appendFileSync('/home/app/listOfVendorsAcc.csv', csvData.map(item => item.ACCESSORIES).join('\n') + '\n');

...
(async () => {
    ...
    try {
        ...
        await page.goto('https://www.cdiscount.com/mp-1538-cds810s.html');
        ...
        const csvData = allContentArray.map(titleContent => ({ 'TABLET': titleContent }));
        console.log(csvData);
        //
        // Vérifiez si le fichier existe
        const filePath = path.resolve('/home/app/listOfVendorsTab.csv');
        ...
        if (!fileExists) {
            fs.writeFileSync(filePath, 'TABLET\n');
        }
        console.log(filePath);
        // Si le fichier existe, append les données
        fs.appendFileSync('/home/app/listOfVendorsTab.csv', csvData.map(item => item.TABLET).join('\n') + '\n');
